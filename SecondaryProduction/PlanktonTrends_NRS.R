...
gg
ggsave("Figures/Zoo_BiomassTrends.png", dpi = 500)
...
#   pm_dat <- pdat2 %>%
#     filter(IMOS_SiteCode==sites[i])
#   pmdl <- lm(BioVol_m3 ~  fHarmonic(HarmDOY, k = 1) + LocalTime, data = pm_dat)
#   p_pval <- anova(pmdl)$'Pr(>F)'[1]
#   tpmdl <- tidy(pmdl)
#   p_Bio_slope[i] <- tpmdl$estimate[tpmdl$term=="LocalTime"] # Save the model slope
#
#   zm_dat <- zdat2 %>% filter(IMOS_SiteCode==sites[i])
#   zmdl <- lm(ZooBiomass_m3 ~ fHarmonic(HarmDOY, k = 1) + LocalTime, data = zm_dat)
#   z_pval <- anova(zmdl)$'Pr(>F)'[1]
#   tzmdl <- tidy(zmdl)
#   z_Bio_slope[i] <- tzmdl$estimate[tzmdl$term=="LocalTime"] # Save the model slope
#
...
#
#   gg <-  ggplot() +
#     geom_point(data = pm_dat, aes(x=LocalTime, y=BioVol_m3), size = pt_size) +
#     stat_smooth(data = pmdl, method = "lm", col = pclr, fill = pclr, aes_string(x = names(pmdl$model)[3], y = names(pmdl$model)[1])) +
#     theme_bw() +
#     scale_y_continuous(limits=c(4, 8), expand = c(0, 0)) +
#     annotate("text", x = as.Date("2009-2-1"), y = 7.6, label = site_names[counter], hjust = 0, size = t_size) +
#     theme(axis.title.x = element_blank(), axis.title.y = element_blank())
#
#   if (i == 7) {
#     gg <- gg +
#       scale_x_date(labels = NULL, date_labels = "%Y", date_breaks = "2 years", date_minor_breaks = "years", limits = c(as.Date("2009-1-1"), as.Date("2020-06-30")), expand = c(0, 0))
#   } else {
#     gg <- gg +
#       scale_x_date(labels = NULL, date_breaks = "2 years", date_minor_breaks = "years", limits = c(as.Date("2009-1-1"), as.Date("2020-06-30")), expand = c(0, 0))
#   }
#
#
#   myplots[[counter]] <- gg
...
#
#   gg <-  ggplot() +
#     geom_point(data = zm_dat, aes(x=LocalTime, y=ZooBiomass_m3), size = pt_size) +
#     stat_smooth(data = zmdl, method = "lm", col = zclr, fill = zclr, aes_string(x = names(zmdl$model)[3], y = names(zmdl$model)[1])) +
#     theme_bw() +
#     annotate("text", x = as.Date("2009-2-1"), y = 2.6, label = site_names[counter], hjust = 0, size = t_size) +
...
#
# myplots[[9]] <- myplots[[9]] +
#   ylab(expression(paste("Phytoplankton Biovolume log"[10],"(",mu,"m m"^{-3},")"))) +
...
#   theme(axis.title.y = element_text(color = "black", size = 12, hjust = 1))
#
# graphics.off()
...
#   pmdl <- lm(Abundance_m3 ~ LocalTime + fHarmonic(HarmDOY, k= 1), data = pm_dat)
#   p_pval <- anova(pmdl)$'Pr(>F)'[1]
#   p_Abun_slope[i] <- pmdl$coefficients[[2]] # Save the model slope
#   tpmdl <- tidy(pmdl)
#   p_Abun_slope[i] <- tpmdl$estimate[tpmdl$term=="LocalTime"] # Save the model slope
#
#   zm_dat <- zdat2 %>% filter(IMOS_SiteCode==sites[i])
#   zmdl <- lm(Abundance_m3 ~ LocalTime + fHarmonic(HarmDOY, k= 1), data = zm_dat)
#   z_pval <- anova(zmdl)$'Pr(>F)'[1]
#   z_Abun_slope[i] <- zmdl$coefficients[[2]]
#   tzmdl <- tidy(zmdl)
#   z_Abun_slope[i] <- tzmdl$estimate[tzmdl$term=="LocalTime"] # Save the model slope
#
#   # Now I calculate the days over a decade and calculate the change
#   decade_length <- as.numeric(difftime(dmy("01-01-2019"), ref_date))
#
...
#
#   gg <-  ggplot() +
#     geom_point(data = pm_dat, aes(x=LocalTime, y=Abundance_m3), size = pt_size) +
#     stat_smooth(data = pmdl, method = "lm", col = pclr, fill = pclr, aes_string(x = names(pmdl$model)[2], y = names(pmdl$model)[1])) +
#     theme_bw() +
#     scale_y_continuous(limits=c(0, 4), expand = c(0, 0)) +
#     annotate("text", x = as.Date("2009-2-1"), y = 3.6, label = site_names[counter], hjust = 0, size = t_size) +
#     theme(axis.title.x = element_blank(), axis.title.y = element_blank())
#
#   if (i == 7) {
#     gg <- gg +
...
#
#   gg <-  ggplot() +
#     geom_point(data = zm_dat, aes(x=LocalTime, y=Abundance_m3), size = pt_size) +
#     stat_smooth(data = zmdl, method = "lm", col = zclr, fill = zclr, aes_string(x = names(zmdl$model)[2], y = names(zmdl$model)[1])) +
#     theme_bw() +
#     annotate("text", x = as.Date("2009-2-1"), y = 4.6, label = site_names[counter], hjust = 0, size = t_size) +
...
#
# myplots[[9]] <- myplots[[9]] +
#   ylab(expression(paste('Phytoplankton Abundance log'[10],'(ind. m'^{-3},')'))) +
#   theme(axis.title.y = element_text(color = "black", size = 12, angle = 90, hjust = 0, vjust = 1))
#
# myplots[[10]] <- myplots[[10]] +
